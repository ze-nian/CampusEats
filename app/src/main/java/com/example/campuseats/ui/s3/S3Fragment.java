package com.example.campuseats.ui.s3;
...
public class S3Fragment extends Fragment {
    ...
    private S3ViewModel mViewModel;
    ...
    public static S3Fragment newInstance() {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        View S3View = inflater.inflate(R.layout.fragment_s3, container, false);
        RootView = S3View;
        ...
        final List<String> namesList = Arrays.asList( "醍醐拉麵套餐", "豚骨拉麵套餐", "咖哩拉麵套餐","魚介拉麵套餐","地獄拉麵套餐");
        final List<Integer> priceList = Arrays.asList(95,100,105,105,110);
        //final List<String> pastList=new ArrayList<>();

        //數量紀錄(不會跳出變0)
        for (int j=2;j<7;j++){
            final int quantity_id=getResources().getIdentifier("s3_"+j, "id", getActivity().getPackageName());
            TextView tt = S3View.findViewById(quantity_id);
            try {
        ...
        //add
        for (int i = 2; i < 7; i++) {
            final int id = getResources().getIdentifier("s3_"+i+"_add", "id", getActivity().getPackageName());
            Button btnAdd = S3View.findViewById(id);

            final int finalI = i;
            btnAdd.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    final int tid = getResources().getIdentifier("s3_"+finalI, "id", getActivity().getPackageName());
        ...
        //minus
        for (int i = 2; i < 7; i++) {
            final int id = getResources().getIdentifier("s3_"+i+"_minus", "id", getActivity().getPackageName());
            Button btnAdd = S3View.findViewById(id);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mViewModel = new ViewModelProvider(this).get(S3ViewModel.class);
